	...
	callback: (message, args, text) => {
		if (!args.length) {
			...
			const spEmbed = new MessageEmbed()
				...
				.setColor('#FFFFFF')
				.setTimestamp();
			message.reply({ embeds: [spEmbed], allowedMentions: { repliedUser: false } });
		}
		else if (args[0].toLowerCase() === 'removeprefix' || args[0].toLowerCase() === 'rprefix') {
			const spEmbed = new MessageEmbed()
		...
		else if (args[0].toLowerCase() === 'rtfm' || args[0].toLowerCase() === 'docs') {
			const docsEmbed = new MessageEmbed()
				.setTitle('Rtfm | Docs')
				.setColor('#FFFFFF')
				.setDescription('\`\`\`yaml\n#Searches something from discord.js stable version documentation and sends it \nUsage: +docs <query>\`\`\`')
				.setTimestamp();
			message.reply({ embeds: [docsEmbed], allowedMentions: { repliedUser: false } });
		}
		else if (args[0].toLowerCase() === 'v13rtfm' || args[0].toLowerCase() === 'v13docs') {
			const docsEmbed = new MessageEmbed()
				.setTitle('Rtfm | Docs')
				.setColor('#FFFFFF')
