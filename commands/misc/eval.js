...
module.exports = {
	...
	callback: async (message, args, text, client) => {
		if (!message.author.id === "564468550727761920") return message.channel.send("cant use this");
		const clean = taxt => {
			if (typeof (taxt) === "string") return text.replaceAll('`', "`" + String.fromCharCode(8203)).replaceAll('@', "@" + String.fromCharCode(8203));
			else return taxt;
		};
		try {
